...
- **SAR Image Generator**（`sar_image_generator.py`）：独立的增强版 SAR 图像生成器，支持层次聚类和更专业的 SAR 报告图像
...
│   └── visualization/            # 可视化
│       ├── __init__.py
│       ├── utils.py              # [v0.2.0] 统一工具: create_sar_table_image, get_activity_color, render_substituent_image...
│       ├── renderer.py           # SARRenderer, PlotSettings (委托表格创建到 utils.py)
│       └── sar_table.py          # 向后兼容包装器 -> 委托给 utils.create_sar_table_image
├── tests/                        # 单元测试
│   ├── __init__.py
...
├── sar_image_generator.py        # 独立 SAR 图像生成器（层次聚类 + 高级 SAR 图）
├── test_render.py                # 快速渲染测试脚本
├── proc.py                       # Excel 去重脚本（按 SMILES 去重，数值列取平均）
...
# 自定义参数
python sar_image_generator.py input.csv output/sar/ --threshold 0.6 --max 8 --min-subs 6
```
